...
export class GoogleSheetsClient {
  ...
  async getRows(range?: string): Promise<any[][]> {
    ...
    const fullRange = range || `${GOOGLE_SHEETS_CONFIG.sheetName}!A${GOOGLE_SHEETS_CONFIG.dataStartRow}:Y`
  ...
  async appendRow(values: any[]): Promise<number> {
    ...
    try {
      ...
      // Extract row number from the update range (e.g., "Workouts!A10:Y10")
